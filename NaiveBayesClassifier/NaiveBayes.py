...
class NaiveBayes:
    ...
    def __init__(self):
        ...
        self.numFolds = 10
        self.posWords = defaultdict(lambda :0)
        self.negWords = defaultdict(lambda :0)
        self.vTotal = set()
        self.vTotalPos = set()
        self.vTotalNeg = set()
    ...
    def classify(self, words):
        ...
        # Write code here
        pPos = math.log(self.total['pos']) - math.log(self.nDoc)
        pNeg = math.log(self.total['neg']) - math.log(self.nDoc)
        posCount = sum(self.posWords.values())
        negCount = sum(self.negWords.values())

        if self.naiveBayesBool:
            # for word in words:
            #     #test for pos
            #     pPos += math.log(self.posWords[word] + 1.5)
            #     pPos -= .5*math.log(posCount + len(self.vTotal))
            #
            #     #test for neg
            #     pNeg += math.log(self.negWords[word] + 1.5)
            #     pNeg -= .5*math.log(negCount + len(self.vTotal))
            for word in words:
                #test for pos
                pPos += math.log(self.posWords[word] + 18)
                pPos -= math.log(posCount + 18*len(self.vTotal))

                #test for neg
                pNeg += math.log(self.negWords[word] + 18)
                pNeg -= math.log(negCount + 18*len(self.vTotal))

        elif self.bestModel:
            for word in words:
                #test for pos
                pPos += math.log(self.posWords[word] + 20)
                pPos -= math.log(posCount + 20*len(self.vTotal))

                #test for neg
                pNeg += math.log(self.negWords[word] + 20)
                pNeg -= math.log(negCount + 20*len(self.vTotal))
        else:
            #81.65%
            for word in words:
                    pPos += math.log(self.posWords[word]  + 1)
                    pPos -= math.log(posCount + len(self.vTotal))
                    #test for neg
                    pNeg += math.log(self.negWords[word]  + 1)
                    pNeg -= math.log(negCount + len(self.vTotal))



        isPos = pPos > pNeg
        sentiment = 'pos' if isPos else 'neg'
        ...
        if self.naiveBayesBool or self.bestModel:
            ...
            if (classifier == 'pos'):
                for word in words:
                    self.posWords[word] += 1
                    self.vTotal.add(word)
                    self.vTotalPos.add(word)
            else:
                for word in words:
                    self.negWords[word] += 1
                    self.vTotal.add(word)
                    self.vTotalNeg.add(word)


        self.total[classifier] += 1
        self.nDoc +=1
