...
# Updated Kevin Jesse Zachary Harris
...
class SpellCorrect:
    ...
    def correctSentence(self, sentence):
        ...
        for i in xrange(1, len(sentence) - 1):  # ignore <s> and </s>
            ...
            # Tip: self.editModel.editProbabilities(word) gives edits and log-probs according to your edit model.
            ...
            wordScore = float('-inf')
            testProbs = self.editModel.editProbabilities((sentence[i]))

            for possibleWord, possibleScore in self.editModel.editProbabilities(sentence[i]):
                #if possibleScore > wordScore:
                    newWord = possibleWord
                    wordScore = possibleScore
                    newSentence = sentence[:i] + [newWord] + sentence[i+1:]
                    score = self.languageModel.score(newSentence) + wordScore
                    if score > bestScore:
